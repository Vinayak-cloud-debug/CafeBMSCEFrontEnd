...
<div className='bg-orange-50 min-h-screen flex flex-col gap-12 px-4 sm:px-8 relative'>
  ...
  <div className='flex flex-wrap justify-center gap-4 sm:gap-6 mt-6'>
    {[
      { label: 'Breakfast', state: nashta, set: () => setNashta(true) },
      { label: 'Lunch', state: oota, set: () => setOota(true) },
      { label: 'Dinner', state: nightOota, set: () => setNightOota(true) },
      { label: 'Drink', state: drinks, set: () => setDrinks(true) },
      { label: 'Dessert', state: icecream, set: () => setIcecream(true) },
      { label: 'Noodles', state: noodle, set: () => setNoodle(true) },
    ].map((btn, i) => (
      <button
        key={btn.label}
        onClick={() => {
          setNashta(false); setOota(false); setNightOota(false);
          setDrinks(false); setIcecream(false); setNoodle(false);
          btn.set(); handleClick(i + 1);
        }}
        className={`px-5 py-2 sm:px-6 sm:py-2.5 rounded-full font-semibold text-base sm:text-lg transition-all duration-300 ease-in-out transform ${
          btn.state
            ? 'bg-gradient-to-r from-red-500 to-orange-500 text-white shadow-lg scale-105'
            : 'bg-white text-gray-800 hover:bg-red-50 hover:text-red-600 shadow'
        }`}
      >
        {btn.label}
      </button>
